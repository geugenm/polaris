...
def sample_entropy(timeseries_data, run_data_length, filtering_level):
    def _phi(run_data_length):
        x = [
             [
                 timeseries_data[j]
                 for j in range(i, i + run_data_length - 1 + 1)]for i in range(
                     N - run_data_length + 1)]
        C = [len([1 for j in range(len(x)) if i != j and _maxdist(x[i], x[j])
             <= filtering_level]) for i in range(len(x))]
        return sum(C)
    ...
    def _phi(run_data_length):
        x = [
             [
                 ...
                 for j in range(i, i + run_data_length - 1 + 1)]for i in range(
                     N - run_data_length + 1)]
        C = [len([1 for x_j in x if _maxdist(x_i, x_j) <= filtering_level]) /
...
#       steps = tf+"("+lag+")"
    ...
    for col in original_features:
        data[col+"_"+pipeline_id+"_"+get_time_lag(tf)] = (
            pipeline.transform(data[col])
        )
    return data
